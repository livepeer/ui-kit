...
<Callout type="warning" emoji="⚠️">
  **This example illustrates how Livepeer can be used as a video processing and caching layer on top of content available on IPFS or the Arweave gateway   for optimized video playback.** As shown below, there are delays when content has never been viewed - the first user will need to wait for the content   to be downloaded from the network and uploaded to the Livepeer player. We recommend, whenever possible, to upload via `useCreateAsset` before       displaying  it in the player.
</Callout>
...
Valid input is passed to the [`Player`](/react/Player), which automatically detects if it is a valid CID and attempts
