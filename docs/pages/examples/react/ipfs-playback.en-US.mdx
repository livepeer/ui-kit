...
<Callout type="warning" emoji="⚠️">
  **This example illustrates how Livepeer can be used as a video processing and
  caching layer on top of IPFS for optimized video playback.** As shown below,
  there are delays when an IPFS CID has never been viewed - the first user will
  need to wait for upload. We recommend, whenever possible, to upload via
