...
Same as before, we create a new livepeer.js client with the Studio provider and a [CORS-protected API key](/docs/providers/studio#apikey):
...
how we do that). Then, we stop polling and display the streaming content using the [`VideoPlayer`](/docs/VideoPlayer).
...
export const Stream = () => {
  ...
  return (
    <>
      ...
      {stream && stream.streamKey && (!stream?.playbackId || !stream.isActive) && (
        <Text>
      ...
      {stream?.playbackId && <VideoPlayer playbackId={stream?.playbackId} />}
    </>
  );
};
```
