...
Lastly, when the stream is created, we use `refetchInterval` to poll the API until the stream becomes active. Then, we stop
polling and display the streaming content in an HLS player.
  ...
  return (
    <Box>
      ...
      <Button
        ...
        disabled={!streamName || createStatus === 'loading' || Boolean(stream)}
      >
        Create Stream
      </Button>
