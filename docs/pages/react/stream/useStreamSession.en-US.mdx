...
The `useStreamSession` hook also comes with a [React Query](https://tanstack.com/query/v4/docs/guides/ssr) prefetch query, `prefetchStreamSession`,
...
you want to prefetch data on. The props should match the `useStreamSession` hook to ensure that the correct data is prefetched.
