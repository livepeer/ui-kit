...
First, you add [`getStaticProps`](https://nextjs.org/docs/basic-features/data-fetching/get-static-props) to the page which
...
We also need to update the `_app.tsx` to pass the `dehydratedState` in `pageProps` to the LivepeerConfig. We also move the
...
That's it! You now have prefetching on the server, which is passed to the user and used to hydrate the initial query client.
