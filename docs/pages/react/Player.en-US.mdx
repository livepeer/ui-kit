...
Sets the React `ref` passed to the underlying media element. Useful when integrating with third party tools, or when access to the underlying video element is needed (usually it isn't!).
...
function PlayerComponent() {
  const mediaElementRef = useRef<Video>();
