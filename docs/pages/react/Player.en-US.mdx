...
<Callout>
  An IPFS [v0 or v1 CID](https://docs.ipfs.tech/concepts/content-addressing/) or IPFS/Arweave URL (such as `ipfs://<CID>`, `https://<CID>.ipfs.dweb.link/` or
  `https://cloudflare-ipfs.com/ipfs/<CID>`, `ar://<HASH>`, `https://<SUBDOMAIN>.arweave.dev/<HASH>` or `https://arweave.net/<HASH>`, _but not a directory_)
  can be passed as the `src` or `playbackID` to the Player, and it will
