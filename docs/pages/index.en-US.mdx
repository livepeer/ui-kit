...
function Livepeer() {
  return <VideoPlayer autoplay loop muted playbackId={playbackId} />;
}
```
...
Then, we use the [`VideoPlayer`](/react/VideoPlayer) to play a video from a playback ID! Waterfalls!
