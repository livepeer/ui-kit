...
<div className="text-center">
  **livepeer.js** is a library to make building with Livepeer easy. It provides a core vanilla JS library to easily connect to a
  Livepeer provider (e.g. Livepeer Studio), as well as React-specific hooks with a similar API to provide memoization and DOM
  management.
...
const assetId = 'a3880c39-b58a-4ae2-8e17-8f3e727ded0d';

function Livepeer() {
  const { data: asset } = useAsset(assetId);

  return <div>Asset name: {asset?.name ?? 'N/A'}</div>;
}
```
...
Next, we use the `useAsset` hook to fetch the asset. Finally, we show the asset's name (and can do many other things with it)!
