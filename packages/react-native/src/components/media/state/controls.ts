import { Storage, noopStorage } from 'livepeer';
...
export type NativeMediaControllerState<TElement extends Video> = {
  ...
  /** Internal Video (or extended class) used for toggling media events */
...
export type MediaControllerStore<TElement extends Video> = StoreApi<
  NativeMediaControllerState<TElement>
>;
...
export const createControllerStore = <TElement extends Video>(
  element: TElement | null,
  storage?: Storage,
) => {
  const store = create<
    NativeMediaControllerState<TElement>,
    [['zustand/persist', Partial<NativeMediaControllerState<TElement>>]]
  >(
    persist(
      (set, get) => ({
        _element: element,
...
export const addEventListeners = <TElement extends Video>(
...
const addEffectsToStore = <TElement extends Video>(
  store: StoreApi<NativeMediaControllerState<TElement>>,
  element: Video | null,
