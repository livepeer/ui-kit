...
import { Source } from "livepeer/dist/models/components";
...
export type UseSourceMimeTypedProps<TElement, TPoster, TSlice> = {
  ...
  playbackInfo: PlayerProps<TElement, TPoster, TSlice>["playbackInfo"];
...
export const useSourceMimeTyped = <TElement, TPoster, TSlice>({
  ...
  const playbackUrls = React.useMemo(() => {
    ...
    const recordingPlaybackSources: PlaybackUrlWithInfo[] = playRecording
      ? (
          playbackInfo?.meta as unknown as {
            dvrPlayback: Source[];
          }
        )?.dvrPlayback?.map((s) => ({
          url: s?.url,
  ...
  }, [playbackInfo, screenWidth, playRecording]);
